...
// Remove trailing position/team/number tokens that often ride along with a name,
// e.g. "Bijan Robinson RB ATL 11" -> "Bijan Robinson". Only strips from the END
// so a real name word is never removed. Bare trailing numbers are bye weeks,
// tiers, ADP or projection columns pasted from a spreadsheet — no NFL player's
// name ends in a number, and a team name that does ("49ers") is not bare digits.
function stripTrailingTags(name) {
  let parts = name.split(/\s+/).filter(Boolean);
  while (parts.length > 1) {
    const last = parts[parts.length - 1];
    if (isPosition(last) || isTeam(last) || isBareNumber(last)) {
      parts.pop();
    } else {
      break;
    }
...
// A token that is nothing but a number (optionally with %, $, a decimal or a
// thousands comma): "11", "6.", "$60", "12.4", "-2.1", "18%".
function isBareNumber(token) {
  return /^[-+$]?\d{1,4}(,\d{3})*(\.\d+)?[%.,)]?$/.test(String(token || ''));
}
