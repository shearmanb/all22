...
function buildList(data) {
  ...
  for (const id in data) {
    ...
    const candidate = {
      ...
      sleeper_id: String(p.player_id || id),
    };
...
async function sync() {
  ...
  syncInflight = (async () => {
    ...
    await pool.query(
      `INSERT INTO players_master (name, name_key, position, team, active, source, sleeper_id)
       SELECT p->>'name', p->>'key', COALESCE(p->>'position',''), COALESCE(p->>'team',''),
              (p->>'active')::boolean, 'sleeper', p->>'sleeper_id'
       ...
       ON CONFLICT (name_key) DO UPDATE
         SET name = EXCLUDED.name,
             ...
             sleeper_id = COALESCE(EXCLUDED.sleeper_id, players_master.sleeper_id),
             updated_at = now()`,
      [JSON.stringify(list)]
    );
...
// Forget a learned alias (Player DB page). Removes the variant from
// players_master.aliases[site]; the live match map rebuilds from the DB on
// the next index build (invalidateIndex), so the in-memory map may keep the
// alias for up to the index TTL — harmless, it only maps to the same player.
async function removeAlias(playerId, variant, site) {
  const siteKey = (site || 'common').toLowerCase().replace(/[^a-z0-9_-]+/g, '-') || 'common';
  const variantKey = players.key(variant || '');
  const { rows } = await pool.query(
    'SELECT name, aliases FROM players_master WHERE player_id = $1', [playerId]
  );
  if (!rows.length) throw new Error('Player not found.');
  const { name, aliases: current } = rows[0];
  const next = Object.assign({}, current);
  const list = (Array.isArray(next[siteKey]) ? next[siteKey] : [])
    .filter((v) => players.key(v) !== variantKey);
  if (list.length) next[siteKey] = list; else delete next[siteKey];
  await pool.query(
    'UPDATE players_master SET aliases = $2, updated_at = now() WHERE player_id = $1',
    [playerId, JSON.stringify(next)]
  );
