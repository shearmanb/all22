...
- `features/playbook/` — draft log & analysis (`/api/drafts`; the old draft tracker, grown into the PRD's Playbook): parsers for Underdog/FantasyPros paste, a FantasyPros Draft Wizard URL fetch (`lib/fpwizard.js` — shape-agnostic JSON hunter, paste stays the fallback), drafts pages. Phase 3 analysis: picks resolve to `player_id` at save (migration 020; high-confidence only, lazily backfilled on first analysis view, force re-match button on the page), and `GET /:id/analysis` grades the draft — value vs ADP (`../adp/lib/boards.js` picks the same board Edge Rush shows; format defaults from the draft's site or the active league profile), steal/reach tags, positional balance, a per-slot value leaderboard, and a "my rank" column from the latest My Rankings run. Pure math in `lib/analysis.js` (golden-tested); steal/reach thresholds are dials in settings `playbook.analysis`, editable on the draft page.
...
- `drafts` + `picks` — Playbook's tables (pre-rebuild; migration 020 adds `picks.player_id` + `match_confidence`/`match_via` so picks resolve through players_master — NULL confidence means never attempted, which is what triggers the lazy backfill).
